...
def executar_diagnostico(self, solicitacao):
    ...
    try:
        ...
        conteudo = (
            ...
            f"acao: diagnostico\n"
            f"data_hora: {momento.strftime('%d/%m/%Y %H:%M:%S')}\n"
            f"origem: {origem}\n"
        ...
        if not os.path.exists(caminho_arquivo):
            ...
            raise RuntimeError(
                "O arquivo de diagnóstico não foi encontrado após a criação."
            )
        ...
        if not conteudo_confirmado.strip():
            ...
            raise RuntimeError(
                "O arquivo de diagnóstico foi criado, mas está vazio."
            )

        if "acao: diagnostico" not in conteudo_confirmado:

            raise RuntimeError(
                "O conteúdo do diagnóstico não foi confirmado corretamente."
            )
        ...
        self.registrar(
            f"Diagnóstico criado e confirmado: {caminho_resultado}"
        )
